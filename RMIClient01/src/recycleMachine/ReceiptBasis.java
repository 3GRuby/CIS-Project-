...
public class ReceiptBasis {
	...
	public void addItem(DepositItem item) { 
		...
		//System.out.println(collectedSize+" - "+RecyclingGUI.maximumSize+" // "+collectedWeight+" - "+RecyclingGUI.maximumWeight);
		if(collectedSize<=RecyclingGUI.maximumSize && collectedWeight<=RecyclingGUI.maximumWeight){
			...
			try {
				...
				servo.addToTransactions(RecyclingGUI.getPublicIpAddress(), item.getClass().getSimpleName(), item.value);
